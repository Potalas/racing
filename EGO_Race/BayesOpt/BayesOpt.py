...
from .InfillCriteria import EI, PI, MGFI, UCB
from .Surrogate import SurrogateAggregation, RandomForest
from .SearchSpace import from_irace
...
class BO(object):
    ...
    def __init__(self, 
        ...
        if self.n_point > 1:
            if self.infill not in ['MGFI', 'UCB']:
                self.infill = 'MGFI'
        ...
        self._levels = np.array([self._space.bounds[i] for i in self._space.id_N], dtype = "object") # discrete
    ...
    def _acquisition(self, plugin=None, dx=False):
        ...
        if self.n_point > 1:  # multi-point method
            ...
            if self.infill == "MGFI":
                ...
                tt = np.random.randn() #snorm.ppf(0.5 + 0.5 * np.random.randn())
                acquisition_func = UCB(self.surrogate, plugin, minimize=self.minimize, alpha=tt)
...
class irace_BO(BO):
    """Interface methods between BO class and R (irace)"""
    def __init__(self, params, **kwargs):
        ...
        ----------
            params: dict
                a dictionary of parameters, as can be passed to
                the 'from_irace' function of the SearchSpace module
            kwargs:
                further agruments to the base BO class
        """
        #TODO: check fixed parameters
        space = from_irace(params)
        levels = space.levels if hasattr(space, 'levels') else None
        super(irace_BO, self).__init__(space, None, RandomForest(levels = levels), max_eval = 100, noisy = True, **kwargs)
    
    def add_data(self, df):
        """
        Wrapper around tell, handles datatable from R,
        
        Parameters
        ----------
            df: DataFrame
                with 'val' column containing the fitness
        """
        flatten = lambda l: [item for sublist in l for item in sublist]
        X = df[flatten(self._space.name)]
        fitness=np.array(df['val'])
        super(irace_BO, self).tell(X, fitness)
    
    def get_candidates(self, nr_cand, seed):
        """
        Wrapper around ask, returns dataframe instead of solution-object
        ...
        ----------
            ...
            seed: int
                Seed for the numpy random generator, useful for 
                the case where R handles parallellization instead of python
        """
        np.random.seed(seed)
        candidates = super(irace_BO, self).ask(nr_cand)
        flatten = lambda l: [item for sublist in l for item in sublist]
        df = pd.DataFrame.from_records(candidates, columns=flatten(self._space.name))
        return df

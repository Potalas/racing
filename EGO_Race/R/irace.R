...
#' @export
irace <- function(scenario, parameters, sample_ego = FALSE, config_factor = NULL, infill = "MGFI")
{
    ...
    if (scenario$maxTime == 0) {
      ...
      while (TRUE) {
        ...
        if (nrow(allConfigurations) < nconfigurations) {
          newConfigurations <- sampleEGO_orig(parameters, NULL, NULL, nbNewConfigurations, 
                                              infill = infill, seed = scenario$seed)
  ...
  while (TRUE) {
    ...
    if (firstRace) {
      ...
      if (nbNewConfigurations > 0) {
        ...
        if (sample_ego){
          newConfigurations <- sampleEGO_orig(parameters, NULL, NULL, nbNewConfigurations, 
                                              infill = infill, seed = scenario$seed, repair = T)
        }
        else {
          newConfigurations <- sampleEGO_orig(parameters, NULL, NULL, nbNewConfigurations, 
                                              infill = infill, seed = scenario$seed)
      ...
      if (is.numeric(config_factor)) {
        nr_threads <- detectCores()
        nbNewConfigurations <- ceiling(nr_threads * config_factor)
        if (nbNewConfigurations * max(scenario$mu, scenario$firstTest) > currentBudget) {
          nbNewConfigurations <- nbConfigurations - nrow(eliteConfigurations)
          print(paste0("Sampling ", nbNewConfigurations, ", according to default setting, since config_factor is too large"))
        }
        else{
          print(paste0("Sampling ", nbNewConfigurations, ", according to config_factor specified"))
        }
      }
      else {
        nbNewConfigurations <- nbConfigurations - nrow(eliteConfigurations)
        print(paste0("Sampling ", nbNewConfigurations, ", according to default setting"))
      }
      # Update the model based on elites configurations
      if (debugLevel >= 1) { irace.note("Update model\n") }
      print("Pre-update")
      if (debugLevel >= 1) { printModel (model) }
      if (sample_ego == 0) 
      ...
      if (sample_ego) {
        #TODO: add digits, forbidden, repair?
        if (!is.na(iraceResults$experimentLog['time']))
          log <- iraceResults$experimentLog
        else {
          log <- reshape2::melt(iraceResults$experiments, na.rm = T)
          setnames(log, c("Var1", "Var2", "value"), c("instance", "configuration", "time"))
        }
       
        newConfigurations <- sampleEGO_orig(parameters, allConfigurations, log, nbNewConfigurations, 
                                            infill = infill, seed = scenario$seed)
      }
      else
      ...
      if (scenario$softRestart && !sample_ego) {
        #          Rprof("profile.out")
        ...
        if (!is.null(tmp.ids)) {
          ...
          #   }
          #   else{
          #     newConfigurations <- sampleEGO(parameters, allConfigurations, log, nbNewConfigurations)
          #   }
          # }
          # else
            newConfigurations <- sampleModel(parameters, eliteConfigurations,
    ...
    repair_conditional <- function(new_cand, parameters) {
      new_cand <- as.data.table(new_cand)
      ...
      rownames(new_cand) <- new_cand$.ID.
      as.data.frame(new_cand)
    }
    if (sample_ego)
      raceConfigurations <- repair_conditional(raceConfigurations, parameters)

...
initialiseModel <- function (parameters, configurations, digits)
{
  ...
  for (currentParameter in parameters$names[!parameters$isFixed]) {
    ...
    for (indexConfig in seq_len(nbConfigurations)) {
      ...
      if (type %in% c("i","r")) {
        value[2] <- as.numeric(configurations[indexConfig, currentParameter])
      }
    ...
    for (currentParameter in parameters$names[!parameters$isFixed]) {
      ...
      if (idCurrentConfiguration  %in% names(oldModel[[currentParameter]])) {
        ...
        if (type %in% c("i", "r") &&
          ...
          probVector[2] <- as.numeric(eliteConfigurations[idCurrentConfiguration,currentParameter])
      }
...
#                 "th.\n")
        ...
        probVector[1] <- as.numeric(probVector[1]) * ((1 / nbNewConfigurations)^(1 / parameters$nbVariable))
      }
